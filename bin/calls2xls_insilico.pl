...
if (!(@ARGV)){
    print "calls2xls_vcf4_insilico.pl <miseq_root> <tidigare remiss-nr> <nytt remiss-nr> <genlista> <frågeställning>\n\n";
    print "Genlistor / frågeställningar:\n";
...
die "calls2xls_vcf4_insilico.pl <miseq_root> <tidigare remiss-nr> <nytt remiss-nr> <genlista> <frågeställning>\n" if ( $#ARGV != 4 );

chomp($miseqroot = $ARGV[0]);
chomp($oldsample = $ARGV[1]);
chomp($newsample = $ARGV[2]);
chomp($genlist = $ARGV[3]);
chomp($inquery = $ARGV[4]);

#$tr{$newsample} = $oldsample;
...
$sampleC{$newsample}{genelist} = $genlist;
$sampleC{$newsample}{disease} = $inquery;
...
foreach $sampledir (@sampledirs) {
    ...
    print "bedtools genomecov -ibam $bam -bga -g $genome -dz | bedtools intersect -wb -a stdin -b $reg | cut -f 1-4,8 | sed \'s/.*/&\t$sample\t$rundate\t$calldate\t$sampleC{$sample}{sex}/\' >$outfile\n";
